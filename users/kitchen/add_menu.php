...
if (isset($_GET['food_id'])) {
    ...
    // Fetch food item details with category information
    $query = "SELECT f.*, c.name as category_name, c.icon as category_icon 
              FROM food_listings f 
              LEFT JOIN food_categories c ON f.category_id = c.category_id 
              WHERE f.food_id = ? AND f.kitchen_id = ?";
    ...
    if ($food_item) {
        echo "<script>
            console.log('Food Item Data:', " . json_encode($food_item) . ");
            console.log('Diet Types:', " . json_encode($food_item['diet_type_suitable']) . ");
            console.log('Health Goals:', " . json_encode($food_item['health_goal_suitable']) . ");
            console.log('Allergens:', " . json_encode($food_item['allergens']) . ");
            console.log('Category:', " . json_encode([
                'id' => $food_item['category_id'],
                'name' => $food_item['category_name'],
                'icon' => $food_item['category_icon']
            ]) . ");
        </script>";
    }
}
                    ...
                    <select id="category_id" name="category_id" required class="form-select">
                        ...
                        if ($category_result && $category_result->num_rows > 0) {
                            while ($category = $category_result->fetch_assoc()) {
                                // Check if we're in edit mode and this is the selected category
                                $selected = '';
                                if ($edit_mode && isset($food_item['category_id'])) {
                                    $selected = ($food_item['category_id'] == $category['category_id']) ? 'selected' : '';
                                }
                        ?>
